            ...
            List {
                ForEach(expenses.items) { item in
                    HStack {
                        ...
                        Text("$\(item.amount)")
                            .padding()
                            .makeAmountStyle(amount: item.amount)
                    }
                }
...
extension View {
    func makeAmountStyle(amount: Int) -> some View {
        if amount <= 10 {
            return modifier(AmountStyle(color: Color.black))
        } else if amount <= 100 {
            return modifier(AmountStyle(color: Color.orange))
        } else {
            return modifier(AmountStyle(color: Color.red))
        }
    }
}
